...
/*   Updated: 2022/03/31 13:48:28 by jgourlin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_3d_draw(t_data *d, float dist, t_3d_dist s, int *img)
{
	...
	{
		if (g.y < g.line_o)
			my_mlx_pixel_put(&d->big_img, s.r, g.y, d->wall->cl);
		else if (g.y > g.line_o && g.y <= g.line_o + g.line_h)
		{	
			my_mlx_pixel_put(&d->big_img, s.r, g.y,
				(int) img[(int)g.ty * d->size + (int) s.tx]);
			g.ty += g.ty_step;
		}
		else
			my_mlx_pixel_put(&d->big_img, s.r, g.y, d->wall->fl);
		g.y++;
	}
}
